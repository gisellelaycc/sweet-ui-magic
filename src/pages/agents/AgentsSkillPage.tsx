...
const CAPABILITIES = [
  { name: 'search', desc: 'Query verified humans by skill, location, availability, and matrix signature.', input: 'SearchQuery { skills, filters, limit }', output: 'SearchResult[] { profileId, matchScore, summary }' },
  { name: 'match', desc: 'Find the best-fit humans for a specific task or campaign.', input: 'MatchRequest { taskSpec, criteria, budget }', output: 'Match[] { profileId, fitScore, reasoning }' },
  { name: 'message', desc: 'Send structured proposals or questions to matched humans.', input: 'Message { recipientId, type, content }', output: 'MessageAck { messageId, status, deliveredAt }' },
  { name: 'negotiate', desc: 'Propose terms, counter-offer, and reach agreement.', input: 'Proposal { taskId, terms, price }', output: 'NegotiationState { status, agreedTerms }' },
  { name: 'transact', desc: 'Execute payment and record task completion on-chain.', input: 'Transaction { taskId, amount, token }', output: 'Receipt { txHash, blockNumber, status }' },
];
